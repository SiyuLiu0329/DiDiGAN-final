...
def training_loop(
    ...
    while True:
        with torch.autograd.profiler.record_function("data_fetch"):
            ...
            phase_real_img, phase_real_c, phase_cls = next(training_set_iterator)
            print(phase_real_c.shape, training_set.constraint_res)

            phase_real_img = (phase_real_img.to(device).to(torch.float32)).split(
                batch_gpu
            )
        ...
        for phase, phase_gen_z in zip(phases, all_gen_z):
            ...
            for real_img, real_c, gen_z, gen_c, clz in zip(
                phase_real_img, phase_real_c, phase_gen_z, phase_gen_c, phase_cls
            ):
