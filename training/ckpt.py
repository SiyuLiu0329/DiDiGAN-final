...
class Ckpt(nn.Module):
    ...
    def forward(self, pred_dir, phase_real_c, n_classes, G_ema, device):
                ...
                for clz in range(n_classes):
                    ...
                    img = (
                        G_ema(
                            z=z,
                            c=c,
                            clz=(torch.ones(1) * clz).long().to(z.device),
                            noise_mode="const",
                        )
                        .cpu()[0][0]
                        .numpy()
                    ...
                    PIL.Image.fromarray(img).save(f"{pred_dir}/c_in={a}_clz={clz}.png")
                    ci = (c - lo) * (255 / (hi - lo))
                    PIL.Image.fromarray(
                        (ci[0, 0, :, :].cpu().numpy()).astype("uint8")
                    ).save(f"{pred_dir}/c_in={a}_src.png")

    def save(self, ckpt_dir, G, G_ema, D):
        if not os.path.exists(ckpt_dir):
            os.makedirs(ckpt_dir)
        for name, m in zip(["G", "D", "G_ema"], [G, D, G_ema]):
            torch.save(m.state_dict(), os.path.join(ckpt_dir, f"{name}.pth"))
