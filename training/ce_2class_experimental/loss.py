...
def save_ncwh_tensor_as_png(prefix: str, tensor: torch.Tensor, postfix: str = None) -> None:
    imgs = tensor.permute(0, 2, 3, 1)
    for i, img in enumerate(imgs):
        if img.shape[-1] == 1:
            img = img[:, :, 0]
        if postfix is not None:
            save_as_rgb_img(f'{prefix}_{i}_{postfix}.png',
                            img.data.cpu().numpy())
        else:
...
class DiDiGANLoss(Loss):
    def __init__(self, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0, pl_batch_shrink=2, pl_decay=0.01, pl_no_weight_grad=False, blur_init_sigma=0, blur_fade_kimg=0, fake_clz_val=-5):
        super().__init__()
        self.device = device
        self.G = G
        self.D = D
    ...
    def run_G(self, z, c, clz, update_emas=False):
        ...
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function('style_mixing'):
                cutoff = torch.empty([], dtype=torch.int256,
                ...
                ws[:, cutoff:] = self.G.mapping(torch.randn_like(
                    z), clz.long(), update_emas=False)[:, cutoff:]
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, clz, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_img2, _ = self.run_G(torch.randn(
                    gen_z.shape, device=gen_z.device), gen_c, clz)
                ...
                loss_seg_g = 1 * \
                    F.mse_loss(seg, F.interpolate(
                        gen_c, (256, 256), mode='bicubic'))
                loss_clz_g = F.cross_entropy(p_clz, clz).mean()
                loss_Gmain = torch.nn.functional.softplus(-gen_logits)
                training_stats.report('Loss/G/loss', loss_Gmain)
            with torch.autograd.profiler.record_function('Gmain_backward'):
                (loss_Gmain.mean().mul(gain) + loss_seg_g.mean()
                 * 1.0 + loss_div + loss_clz_g).backward()
                print("____G____")
                print(
        ...
        if phase in ['Greg', 'Gboth']:
            with torch.autograd.profiler.record_function('Gpl_forward'):
                ...
                if count == 10:
                    ...
                    loss_Gmain_inter = torch.nn.functional.softplus(-inter_logits).mean()
                    loss_seg_g_inter = 1 * F.mse_loss(inter_seg, F.interpolate(inter_c, (256, 256), mode='bicubic')).mean()
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(
                    gen_z, gen_c, clz, update_emas=True)
                gen_logits, seg, p_clz = self.run_D(
                    gen_img, gen_c, blur_sigma=blur_sigma, update_emas=True)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_seg_d_fake = torch.zeros_like(seg) #1 * F.mse_loss(seg, torch.ones_like(seg) * self.fake_clz_val)
                loss_clz_d_fake = F.cross_entropy(
                    p_clz, (torch.ones_like(clz) * 2).long()).mean()
                loss_Dgen = torch.nn.functional.softplus(gen_logits)
            with torch.autograd.profiler.record_function('Dgen_backward'):
                (loss_Dgen.mean().mul(gain) + loss_seg_d_fake.mean()
                 * 1.0 + loss_clz_d_fake).backward()
                print("____D(F)____")
                print(
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if phase in ['Dmain', 'Dboth']:
                    loss_seg_d_real = 1 * \
                        F.mse_loss(seg, F.interpolate(
                            real_c, (256, 256), mode='nearest')).mean()
                    loss_clz_d_real = F.cross_entropy(p_clz, clz).mean()
            ...
            with torch.autograd.profiler.record_function(name + '_backward'):
                ((loss_Dreal + loss_Dr1).mean().mul(gain) +
                 loss_seg_d_real * 1.0 + loss_clz_d_real).backward()