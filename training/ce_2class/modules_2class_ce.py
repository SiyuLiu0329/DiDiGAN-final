...
class SynthesisInput(torch.nn.Module):
    ...
    def forward(self, w):
        ...
        x = (grids.unsqueeze(3) @ freqs.permute(0, 2,
             1).unsqueeze(1).unsqueeze(2)).squeeze(3)
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        stopbands = first_stopband * \
            (last_stopband / first_stopband) ** exponents
        ...
        for idx in range(self.num_layers + 1):
            ...
            layer = SynthesisLayer(
                ...
                in_channels=int(channels[prev] + 1), out_channels=int(channels[idx]),
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, clz, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, clz, truncation_psi=truncation_psi,
                          truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        img = self.synthesis(
            ws, c, update_emas=update_emas, **synthesis_kwargs)
        return img
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        self.b4 = DiscriminatorEpilogue(
            channels_dict[4], cmap_dim=0, resolution=4, **epilogue_kwargs, **common_kwargs)
        ...
        self.clz = torch.nn.Sequential(
            ...
            FullyConnectedLayer(512, 3)
        )

    def forward(self, img, update_emas=False, return_features_only=False, **block_kwargs):
